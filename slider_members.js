...
const sliderImages = document.querySelector('.members__carousel');
//Находим кнопку назад
const participantPrevButton = document.querySelector('.members__swiper-arrow-prev');
//Находим кнопку вперёд
const participantNextButton = document.querySelector('.members__swiper-arrow-next');
...
const paticipantsSlide = () => {
  ...
  if (window.screen.width >= 1365) {
  document.querySelector('.members__counter').textContent = paticipantSlideIndex+3;
  } else document.querySelector('.members__counter').textContent = paticipantSlideIndex+1;
}
...
//При увеличении размера вьюпорта выше 480px и обратно переставляем слайдер в начальное положение
window.addEventListener('resize', () => {
  if (window.matchMedia('(min-width: 1365px)').matches) {
    ...
    document.querySelector('.members__count').textContent = participantSlideCount;
    participantSlideCount = participantSlideCount-2;
  } else {
    paticipantSlideIndex = 0;
    paticipantChangeSlide(0);
...
document.querySelector('.members__count').textContent = participantSlides.length ;
